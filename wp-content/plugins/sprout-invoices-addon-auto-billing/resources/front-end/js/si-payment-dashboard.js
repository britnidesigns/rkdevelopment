;(function( $, si, undefined ) {
	...
	si.paymentDashboard.init = function() {
		...
		$('.cim_delete_card').on( 'click', function(event){
			...
			$.post( si_js_object.ajax_url, { action: 'si_ab_card_mngt', cim_action: 'remove_payment_profile', remove_profile: $payment_profile, invoice_id: $client_id }, // sending client id instead of invoice_id; CIM will figure it out.
				function( data ) {
		...
		$('.one_click_payment').on( 'click', function(event){
			event.preventDefault();
			...
			$.post( si_js_object.ajax_url, { action: 'si_attempt_auto_charge', client_id: client_id, invoice_id: invoice_id, nonce: nonce },
				function( response ) {
					$payment_button.hide();
					jQuery('.spinner').hide();
		...
		$('#show_payment_sources_management').on( 'click', function(event){
			$(this).hide();
			$('#client_billing_fields').show();
		});
	};
