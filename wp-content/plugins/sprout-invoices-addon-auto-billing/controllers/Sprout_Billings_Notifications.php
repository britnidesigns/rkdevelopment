 ...
 *
 *
 * @package SI_Sprout_Billings_Notifications
 */
class SI_Sprout_Billings_Notifications extends SI_Notifications {
	...
	public static function register_notifications( $notifications = array() ) {
		$default_notifications = array(
				...
				'auto_billing_profile_not_setup' => array(
					'name' => __( 'Auto Payments Not Setup' , 'sprout-invoices' ),
					'description' => __( 'Customize the email that is sent to the client when an automatic payment is attempted but their profile is not setup.' , 'sprout-invoices' ),
					...
					'default_title' => sprintf( __( 'Automatic Payments Not Setup at %s' , 'sprout-invoices' ), get_bloginfo( 'name' ) ),
					'default_content' => self::default_auto_billing_profile_not_setup(),
					'default_disabled' => true,
				),
				'auto_billing_payment_failed' => array(
					'name' => __( 'Auto Payments Failed' , 'sprout-invoices' ),
					'description' => __( 'Customize the email that is sent to the client when an automatic payment fails.' , 'sprout-invoices' ),
					'shortcodes' => array( 'date', 'name', 'username', 'admin_note', 'line_item_table', 'line_item_list', 'line_item_plain_list', 'invoice_subject', 'invoice_id', 'invoice_edit_url', 'invoice_url', 'invoice_issue_date', 'invoice_due_date', 'invoice_past_due_date', 'invoice_po_number', 'invoice_tax_total', 'invoice_tax', 'invoice_tax2', 'invoice_total', 'invoice_subtotal', 'invoice_calculated_total', 'invoice_deposit_amount', 'invoice_total_due', 'invoice_total_payments', 'client_name', 'auto_bill_error' ),
					'default_title' => sprintf( __( 'Automatic Payment Recently Failed at %s' , 'sprout-invoices' ), get_bloginfo( 'name' ) ),
					'default_content' => self::default_auto_billing_payment_failed(),
				),
				// Admin Notification
				'admin_auto_billed_payment_failed' => array(
					'name' => __( 'Admin Notification: Auto Billing Failed' , 'sprout-invoices' ),
					'description' => __( 'Customize the email that is sent to you when an automatic payment fails.' , 'sprout-invoices' ),
					...
					'default_title' => sprintf( __( '%s: Auto Billing Failed' , 'sprout-invoices' ), get_bloginfo( 'name' ) ),
	...
	public static function add_notification_shortcode( $default_shortcodes = array() ) {
		$new_shortcodes = array(
			'auto_bill_error' => array(
				'description' => __( 'Used to show the error from the auto billing API response.' , 'sprout-invoices' ),
	...
	private static function notification_hooks() {
		...
		// Admin
		add_action( 'sb_attempt_charge_failed', array( __CLASS__, 'failed_payment_notification' ), 10, 5 );
	}

	public static function failed_payment_notification( $payment_profile_id, $payment_id, $invoice_id, $client_id, $failed_attempts ) {

		// don't resend notifications since this wouldn't be the first failed attempt
		if ( count( $failed_attempts ) >= apply_filters( 'sb_failed_attempts_count', 1 ) ) {

			// maybe just send a notification to the admin.
			self::send_admin_notification_about_auto_bill_failed( sprintf( __( 'Last Failed Payment Attempt: %s', 'sprout-invoices' ), $payment_id ), $invoice_id, $client_id );
	...
	public static function default_auto_billing_profile_not_setup() {
		...
		return SI_Sprout_Billings::load_addon_view_to_string( $path . 'auto-billed-not-setup', array(), true );
	}
	...
	public static function default_auto_billing_payment_failed() {
		...
		return SI_Sprout_Billings::load_addon_view_to_string( $path . 'auto-billed-failed', array(), true );
	}
	...
	public static function default_automatic_billing_failed_to_admin() {
		...
		return SI_Sprout_Billings::load_addon_view_to_string( $path . 'admin-auto-billing-failed', array(), true );
	}
